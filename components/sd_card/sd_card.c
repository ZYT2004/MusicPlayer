        ...
        { // Only process regular files
            ...
            if (stat(files[*file_count].name, &st) == 0)
            {
                ...
                if (f != NULL)
                {
                    ...
                    // 动态分配内存
                    files[*file_count].content = (uint8_t *)malloc(file_size);
                    if (files[*file_count].content == NULL)
                    {
                    ...
                    // 读取文件内容
                    size_t bytes_read = fread(files[*file_count].content, 1, file_size, f);
                    files[*file_count].content_size = bytes_read; // 存储实际读取的字节数
                    ESP_LOGE(TAG, "content_size:%zu", bytes_read);
                    fclose(f);
                }
            }
            (*file_count)++;
